...
public class ManuscriptClient implements TimeTracking<TimeTrackingRequest, ManuscriptApiResponse> {
  ...
  private static final DateTimeFormatter MANUSCRIPT_DATE = DateTimeFormatter
  ...
  /**
   * Attempts to log the user into the Manuscript API. Requires either email address/full name, and
   * the password. If successfully logged in a token will be returned.
   * 
   * @param email
   *          represents either the email address/full name that will be utilized to log in this
   *          user
   * @param password
   ...
   *           If the email, and password parameters do not authenticate against the API
   *           successfully.
  ...
  public String login(final String email, final String password, final String baseUrl)
    ...
    ResponseEntity<ManuscriptApiResponse> response = restTemplate.exchange(baseUrl + "/api/logon",
        HttpMethod.POST, request, ManuscriptApiResponse.class);
    ...
    for (com.pomodoro.integration.manuscript.domain.Error error : response.getBody().getErrors()) {
      if (StringUtils.isNotBlank(error.getMessage())
          && error.getMessage().contains(LOGIN_FAILURE)) {
  ...
  /**
   * Determines if the user is logged into the manuscript API. The token is used to perform this
   * validation.
   * 
   * @param token
  ...
  /**
   * Adds an estimate of time(hours) to a given case. This estimate is required to perform time
   * tracking.
   * 
   * @param token
   *          represents the authentication of a user for the API.
   * @param id
   *          represents the case/bug id, estimate will be added to this case.
   * @param hours
   *          represents the amount of time the user believe this case will take.
   * @param baseUrl
   *          represents the base url, determines where the API is located.
   * @return the response from the manuscript API, only returned for convenience.
   * @throws AuthenticationException
   *           thrown if the user is not logged in/invalid token.
   */
  public ResponseEntity<ManuscriptApiResponse> provideCaseEstimate(final String token,
      final String id, final Integer hours, final String baseUrl) throws AuthenticationException {
    ...
    if (isLoggedIn(token, baseUrl)) {
      ...
      HttpEntity<Map<String, String>> request = new HttpEntity<>(body);
      ResponseEntity<ManuscriptApiResponse> response = restTemplate.exchange(baseUrl + "/api/edit",
          HttpMethod.POST, request, ManuscriptApiResponse.class);
      return response;
    } else {
      throw new AuthenticationException();
   ...
   * 
   * @param searchParam
   *          can represent the case or case title, determines what is being searched for. If case
   *          is provided only a single result should be returned.
   * @param token
   *          represents the authentication of a user for the API.
   * @param baseUrl
   *          represents the base url, determines where the API is located.
   * @return A list of cases, these cases would represent both the case title and case number.
   */
  @SuppressWarnings("unchecked")
  public List<Case> searchForCase(final String searchParam, final String token,
    ...
    if (isLoggedIn(token, baseUrl)) {
      ...
      HttpEntity<Map<String, String>> request = new HttpEntity<>(body);
      ResponseEntity<ManuscriptApiResponse> response = restTemplate
          .exchange(baseUrl + "/api/search", HttpMethod.POST, request, ManuscriptApiResponse.class);
  ...
  @Override
  public ResponseEntity<ManuscriptApiResponse> startTimeTracking(
      TimeTrackingRequest timeTrackingRequest) throws AuthenticationException {
    if (isLoggedIn(timeTrackingRequest.getToken(), timeTrackingRequest.getBaseUrl())) {
      ...
      ResponseEntity<ManuscriptApiResponse> response = restTemplate.exchange(
          timeTrackingRequest.getBaseUrl() + "/api/startWork", HttpMethod.POST, request,
  ...
  @Override
  public ResponseEntity<ManuscriptApiResponse> stopTimeTracking(
      TimeTrackingRequest timeTrackingRequest) throws AuthenticationException {
    if (isLoggedIn(timeTrackingRequest.getToken(), timeTrackingRequest.getBaseUrl())) {
      HttpEntity<TimeTrackingRequest> request = new HttpEntity<>(timeTrackingRequest);
      ResponseEntity<ManuscriptApiResponse> response = restTemplate.exchange(
          timeTrackingRequest.getBaseUrl() + "/api/stopWork", HttpMethod.POST, request,
          ManuscriptApiResponse.class);
      return response;
    } else {
      throw new AuthenticationException();
  ...
  @Override
  public ResponseEntity<ManuscriptApiResponse> createTimeTrackingInterval(
      TimeTrackingRequest timeTrackingRequest, Action action) {
    if (isLoggedIn(timeTrackingRequest.getToken(), timeTrackingRequest.getBaseUrl())) {
      Map<String, String> body = new HashMap<>();
      body.put("ixBug", timeTrackingRequest.getIxBug());
      body.put("dtStart",
          action.getStartTime().withZoneSameInstant(ZoneOffset.UTC).format(MANUSCRIPT_DATE));
      body.put("dtEnd",
          action.getEndTime().withZoneSameInstant(ZoneOffset.UTC).format(MANUSCRIPT_DATE));
      body.put("token", timeTrackingRequest.getToken());

      HttpEntity<Map<String, String>> request = new HttpEntity<>(body);
      ResponseEntity<ManuscriptApiResponse> response = restTemplate.exchange(
          timeTrackingRequest.getBaseUrl() + "/api/newInterval", HttpMethod.POST, request,
          ManuscriptApiResponse.class);
      return response;
    } else {
      throw new AuthenticationException();
